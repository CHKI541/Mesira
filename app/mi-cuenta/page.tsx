...
function MiCuentaContent() {
  ...
  // PWA alert preferences
  const [alertPref, setAlertPref] = useState<"all" | "custom">("all");
  const [alertNotifyByEmail, setAlertNotifyByEmail] = useState(true);
  const [alertNotifyByPush, setAlertNotifyByPush] = useState(true);
  ...
  // Sync PWA alert preferences when user profile loads
  useEffect(() => {
    if (user) {
      setAlertPref(user.alertPreference || "all");
    }
  }, [user]);
  ...
  const handleCreateAlert = async (e: React.FormEvent) => {
    ...
    // Validation: at least one notification channel must be selected
    if (!alertNotifyByEmail && !alertNotifyByPush) {
      setDashboardError("Seleccioná al menos un canal de notificación: por email o por notificación push.");
      return;
    }
    ...
    try {
      const newAlert = await createAlert({
        ...
        conditions,
        neighborhoods,
        notifyByEmail: alertNotifyByEmail,
        notifyByPush: alertNotifyByPush
      } as any);
      ...
      setAlertNotifyByEmail(true);
      setAlertNotifyByPush(true);

      const channelDesc = alertNotifyByEmail && alertNotifyByPush
        ? "por email y notificaciones push"
        : alertNotifyByEmail
          ? "por email"
          : "por notificación push en tu dispositivo";
      setSuccessMessage(`¡Alerta creada con éxito! Te avisaremos ${channelDesc} cuando haya coincidencias.`);
  ...
  const handleToggleAlertPreference = async (pref: "all" | "custom") => {
    ...
    try {
      ...
      setSuccessMessage("Preferencia de alertas actualizada con éxito.");
      setTimeout(() => setSuccessMessage(null), 3000);
    } catch (err) {
      console.error("Error updating alert preference:", err);
      setDashboardError("No se pudo actualizar la preferencia de alertas.");
    }
  };
    ...
    <div className="flex flex-col lg:flex-row gap-6 items-start">
      ...
      {activeTab === "alertas" && (
        <div className="space-y-6">
          {/* PWA Alerts Configuration & Status */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Global Alert Preference Card */}
            <div className="bg-white border border-ml-border rounded-xl p-5 shadow-sm">
              <h3 className="text-xs font-black text-ml-dark uppercase tracking-wider mb-2 flex items-center gap-1.5">
                <Bell className="text-[#0043C6]" size={16} />
                <span>¿Qué alertas querés recibir en tu celu?</span>
              </h3>
              <p className="text-xs text-gray-500 mb-4 leading-relaxed">
                Elegí si querés recibir notificaciones push de absolutamente todos los regalos, o solo de tus alertas personalizadas de abajo.
              </p>
              ...
              <div className="flex flex-col gap-2.5">
                <button
                  ...
                  className={`flex items-start gap-2.5 text-left p-3.5 rounded-xl border text-xs transition cursor-pointer ${
                    alertPref === "all"
                      ? "bg-cyan-50/40 border-cyan-300 text-cyan-800"
                      : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50"
                  }`}
                >
                  <span className={`w-4 h-4 rounded-full border shrink-0 mt-0.5 flex items-center justify-center ${alertPref === "all" ? "border-[#0043C6] bg-[#0043C6]" : "border-gray-450"}`}>
                    {alertPref === "all" && <span className="w-1.5 h-1.5 rounded-full bg-white"></span>}
                  </span>
                  <div>
                    <span className="font-extrabold block text-sm">Recibir TODO (Predeterminado)</span>
                    <span className="text-xs text-gray-500 mt-0.5 leading-relaxed block font-medium">
                      Te notificaremos al instante de cada nuevo regalo subido a la web.
                    </span>
                  </div>
                </button>
                ...
                >
                  <span className={`w-4 h-4 rounded-full border shrink-0 mt-0.5 flex items-center justify-center ${alertPref === "custom" ? "border-[#0043C6] bg-[#0043C6]" : "border-gray-450"}`}>
                    {alertPref === "custom" && <span className="w-1.5 h-1.5 rounded-full bg-white"></span>}
                  </span>
                  <div>
                    <span className="font-extrabold block text-sm">Solo mis Alertas</span>
                    <span className="text-xs text-gray-500 mt-0.5 leading-relaxed block font-medium">
                      Te notificaremos solo si el regalo coincide con tus filtros guardados abajo.
                    </span>
                  </div>
                </button>
              </div>
          ...
          <div className="lg:col-span-7 bg-white border border-ml-border rounded-lg p-6 shadow-sm">
            ...
            <form onSubmit={handleCreateAlert} className="space-y-5">
              ...
              <div className="pt-3 border-t border-gray-150">
                <label className="block text-xs font-bold text-ml-dark uppercase tracking-wider mb-2.5">
                  ¿Por qué canales querés recibir esta alerta?
                </label>
                <div className="flex flex-col sm:flex-row gap-3">
                  <label className="flex items-center gap-2.5 p-3 rounded-lg border text-xs font-semibold cursor-pointer select-none bg-white border-gray-200 hover:bg-gray-50 text-gray-700 flex-1">
                    <input 
                      type="checkbox" 
                      checked={alertNotifyByEmail} 
                      onChange={(e) => setAlertNotifyByEmail(e.target.checked)}
                      className="rounded border-gray-300 text-cyan-600 focus:ring-cyan-500/20 w-4 h-4 cursor-pointer"
                    />
                    <span>Por Email ({user?.email})</span>
                  </label>
                  
                  <label className="flex items-center gap-2.5 p-3 rounded-lg border text-xs font-semibold cursor-pointer select-none bg-white border-gray-200 hover:bg-gray-50 text-gray-700 flex-1">
                    <input 
                      type="checkbox" 
                      checked={alertNotifyByPush} 
                      onChange={(e) => setAlertNotifyByPush(e.target.checked)}
                      className="rounded border-gray-300 text-cyan-600 focus:ring-cyan-500/20 w-4 h-4 cursor-pointer"
                    />
                    <span>Notificación Push en el Celu</span>
                  </label>
                </div>
              </div>
